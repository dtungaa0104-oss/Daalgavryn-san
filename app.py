...
def init_db():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS questions (
        ...
        difficulty TEXT NOT NULL, bloom TEXT NOT NULL, q_type TEXT NOT NULL,
...
def blueprint_page():
    return render_template("blueprint.html", subjects=SUBJECTS,
        exam_types=EXAM_TYPES, difficulties=DIFFICULTY,
...
def api_questions():
    ...
    difficulty=request.args.get("difficulty","all"); bloom=request.args.get("bloom","all")
...
def generate_exam():
    ...
    for diff in ["Хялбар","Дунд","Хүнд"]:
        ...
        rows=conn.execute(
            ...
            (grade,subject,diff,cnt)).fetchall()
...
def admin_dashboard():
    ...
    conn.close()
    return render_template("admin_dashboard.html",total=total,by_diff=by_diff,
        by_sub=by_sub,recent=recent,subjects=SUBJECTS,difficulties=DIFFICULTY,blooms=BLOOM,q_types=Q_TYPES)

@app.route("/admin/add",methods=["GET","POST"])
@login_required
def admin_add():
    if request.method=="POST":
        f=request.form; diff=f["difficulty"]
        score=1 if diff=="Хялбар" else(2 if diff=="Дунд" else 3)
        q_code=f"Q{f['grade']}-{f['subject'][:3]}-{datetime.now().strftime('%f')}"
        conn=get_db()
        conn.execute("""INSERT INTO questions(q_code,grade,subject,difficulty,bloom,q_type,question,
...
def admin_edit(qid):
    conn=get_db()
    if request.method=="POST":
        f=request.form; score=1 if f["difficulty"]=="Хялбар" else(2 if f["difficulty"]=="Дунд" else 3)
...
@login_required
def admin_list():
    grade=request.args.get("grade",""); subject=request.args.get("subject",""); diff=request.args.get("difficulty","")
    conn=get_db(); sql,params="SELECT * FROM questions WHERE 1=1",[]
    if grade:   sql+=" AND grade=?";      params.append(int(grade))
    if subject: sql+=" AND subject=?";    params.append(subject)
    ...
    rows=conn.execute(sql,params).fetchall(); conn.close()
    return render_template("admin_list.html",questions=rows,subjects=SUBJECTS,difficulties=DIFFICULTY,
...
def admin_import():
    if request.method=="POST":
        ...
        subject=request.form.get("subject","Математик"); diff=request.form.get("default_difficulty","Дунд")
        ...
        try:
            ...
            questions=parse_raw_text(raw,grade,subject,diff)
            if not questions:
                return jsonify({"error":"Даалгавар илрүүлж чадсангүй. Файлын форматыг шалгана уу."}),400
            conn=get_db(); saved=skipped=0
            for q in questions:
                try:
                    conn.execute("""INSERT INTO questions(q_code,grade,subject,difficulty,bloom,q_type,
                        question,option_a,option_b,option_c,option_d,answer,score,topic)
                        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                        (q['q_code'],q['grade'],q['subject'],q['difficulty'],q['bloom'],q['q_type'],
    ...
    return render_template("admin_import.html",subjects=SUBJECTS,difficulties=DIFFICULTY)
...
def admin_ai_generate():
    if request.method=="POST":
        ...
        topic=request.form.get("topic",""); diff=request.form.get("difficulty","Дунд")
...
Блумын ангилал: {bloom} | Хүндрэл: {diff} | Төрөл: {q_type}
...
Монгол хэлээр."""
        try:
            ...
            raw_text=re.sub(r'^```json\s*','',raw_text); raw_text=re.sub(r'^```\s*','',raw_text)
            ...
            for idx,q in enumerate(ai_qs):
                ...
                try:
                    conn.execute("""INSERT INTO questions(q_code,grade,subject,difficulty,bloom,q_type,
                        question,option_a,option_b,option_c,option_d,answer,score,topic)
                        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                        (q_code,grade,subject,diff,bloom,q_type,q.get('question',''),
